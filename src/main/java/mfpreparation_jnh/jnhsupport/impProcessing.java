package mfpreparation_jnh.jnhsupport;
...
public class impProcessing {
					...
					for(int s = 0; s < imp.getNSlices(); s++){
						for(int t = 0; t < imp.getNFrames(); t++){
							if(imp.getStack().getVoxel(x, y, imp.getStackIndex(c+1, s+1, t+1)-1) 
									> impMax.getStack().getVoxel(x, y, imp.getStackIndex(c+1, s+1, 1)-1)){
								impMax.getStack().setVoxel(x, y, imp.getStackIndex(c+1, s+1, 1)-1,
	...
	public static ImagePlus averageIntensityProjection(ImagePlus imp){
		ImagePlus impMax = IJ.createImage("average projection", imp.getWidth(), imp.getHeight(), 1, imp.getBitDepth());
		double average;
		for(int x = 0; x < imp.getWidth(); x++){
			for(int y = 0; y < imp.getHeight(); y++){
				average = 0.0;
	...
	public static ImagePlus stackSum(ImagePlus imp){
		ImagePlus impMax = IJ.createImage("sum", imp.getWidth(), imp.getHeight(), 1, imp.getBitDepth());
		double sum;
		for(int x = 0; x < imp.getWidth(); x++){
			for(int y = 0; y < imp.getHeight(); y++){
				sum = 0.0;
	...
	public static ImagePlus splitToStack(ImagePlus imp){
			...
			if(frames > 1 && slices == 1 || (frames == 1 && slices == 1)){			
